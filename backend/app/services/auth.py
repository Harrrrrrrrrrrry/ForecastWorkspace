...
class AuthService:
        ...
        *,
        ...
        try:
            with self._connect() as connection:
                ...
                user_id = self._last_inserted_id(cursor)
    ...
    def _bootstrap_owner_account(self, connection: Any) -> None:
    ...
    def _delete_expired_tokens(self, connection: Any) -> None:
        connection.execute(
            self._sql("DELETE FROM auth_tokens WHERE expires_at <= ?"),
            (self._utcnow().isoformat(),),
        )

    def _row_to_user(self, row: Any) -> AuthUser:
        return AuthUser(
            id=int(row["id"]),
            email=str(row["email"]),
    ...
    def _last_inserted_id(self, cursor: Any) -> int:
        if not self._uses_postgres:
            return int(cursor.lastrowid)

        row = cursor.fetchone()
        if row is None:
            raise RuntimeError("Postgres insert did not return a user id.")
        return int(row["id"])

    def _rowcount(self, cursor: Any) -> int:
